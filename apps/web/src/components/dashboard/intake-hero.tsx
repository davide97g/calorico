...
interface IntakeHeroProps {
  consumed: number
  target: number
  /** Top of the acceptable band: past this, the gauge turns red. */
  max?: number
...
export function IntakeHero({
  ...
  // Past the top of the acceptable band is the only state worth colouring red;
  // merely past the target still sits inside the band.
  const pastBand = consumed > (max ?? target)

  return (
    <section className="bg-primary text-primary-foreground shadow-float relative overflow-hidden rounded-[32px] p-5">
        ...
        <div className="bg-primary-foreground/15 relative h-3 rounded-full">
          <span
            className={cn(
              ...
              pastBand ? 'bg-destructive' : 'bg-primary-foreground',
            )}
            style={{ width: `${fill}%` }}
          />
